...
See [GDK.md](GDK.md) for the kit contents, current capabilities, proposed commands and acceptance criteria. This work packages the lessons from the sample games into reusable creator tools; it does not mark runtime/distribution feasibility complete.
...
- [ ] Deliver the separate controller-operated **Giga Couch** player app: TV library, launch/return, profiles, account and social flows. See [player-app specification](docs/giga-couch-app.md); current source library is a prototype, not the full hosted account/social product.
- [ ] Local creator preview: documented semantic actions, reusable lobby/menu/lifecycle, local saves, independent 2D/3D starters, generic project playtest, structured checks and AI onboarding.
...
- [x] Native macOS USB reader for vendor-class Xbox 360 XID pads: device catalog, generic `ff:5d:01` match, multi-pad JSON, SDK consumer, synthetic decode/lookup/input checks. Nacon Compact (`146b:0603`) physically joins Little World on this Mac. HID DualShock/Xbox pads stay on Godot/SDL. GIP, rumble, and touchpad are out of scope. See `docs/wired-usb-controllers.md`.
