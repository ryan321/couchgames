...
Pair wireless controllers with the computer before joining; no cable is required for a compatible Bluetooth controller. Wired DualShock / Xbox HID pads join normally. Vendor-class Xbox 360-style USB pads (including the Walmart Nacon compact) use the host USB helper: `python3 scripts/play.py` starts it when such a pad is plugged in. Keep the wired USB reader window open. F3 shows whether the helper is live. [Wired USB helper](../../../docs/wired-usb-controllers.md). [Setup and hardware test matrix](../../../docs/controller-test-matrix.md).
