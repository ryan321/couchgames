...
Local verification on macOS ARM64 with the installed Godot 4.7.2: **72 Sunbreak assertions**, **20 library assertions**, and **8 Python host tests** passed. Forward+ / Metal gameplay and menu captures were visually inspected. The earlier full SDK suite passed. The latest run passes all 72 Sunbreak checks, then stops on a resource-in-use-at-exit error in the concurrently developed Gauntlet audio test. Gauntlet is being edited separately and was not modified for this task. No Rust files, runtimes, or export templates were changed.

Input regression checks cover free-pointer deploy/resume, controller selection before menu event consumption, stale deflected axes, ten seconds of neutral/drift input, both vertical look directions, mouse/controller isolation, and R3 recentering. The connected Xbox One S reproduced false upward input while moving, jumping, or shooting with the right stick untouched. Both SDL HIDAPI and Apple GCController produced the signal, so the unsuccessful launcher backend override was removed. Four additional real-player-tick checks confirm that movement, jumping, and shooting preserve camera pitch when right-stick input is neutral, and that look stops on release while movement/fire continue. This does not resolve the physical-controller fault; independent native diagnosis is in progress. See [input diagnosis](../../../docs/sunbreak-art.md#xbox-release-drift-diagnosis).
