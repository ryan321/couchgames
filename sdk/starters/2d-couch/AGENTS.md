...
var lobby = Platform.install_lobby()    # join/ready overlay; lobby.match_started
```
...
```sh
couch check --project . --json
couch run --project .
couch pack --project .                 # needs already-installed Godot export templates; never downloads them
couch publish --project . --visibility private   # local outgoing folder only; no account upload yet
```

Do not invent a store upload. Do not download export templates unless the human explicitly asks.
