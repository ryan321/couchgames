...
## Blender cast revision

The hero rigs now wear original Blender-authored armor, leatherwork, wizard ornaments and four class-specific weapons, a domed Warrior shield and a smaller Valkyrie kite shield. Existing skeletons, locomotion/combat blending, class proportions, player colors, fitted headgear and trailing wizard cloth remain. The three primitive creature types are replaced with a hooded wraith, armored raider and horned demon. Shared vertex-colored mesh parts bound each creature to three, seven or eight surfaces, with instance-local limb motion and actual combat-event attack poses. Damage flashes and temporary health bars continue to follow each creature.

Editable source, reproducible generation commands and rendering details are in the [cast asset notes](../sdk/examples/gauntlet/assets/cast/README.md). The [live Godot cast preview](../art/gauntlet/cast/in_game_cast.png) uses the same hero/enemy factories as gameplay. Asset/animation tests cover shared geometry, imported pivots, pause, settling and attack events. Full-suite verification is recorded in the implementation checklist. No additional downloads were required beyond the previously authorized Blender installation.

## Dungeon polish and temporary party fog

The next Blender pass adds engraved armor/trim, stitched ornament and leather pouches. Hero bodies lean with measured movement and breathe subtly; ground creatures bend their knees, turn smoothly and shift their weight, while wraiths drift and demon tails move independently. The Warrior uses an authored horizontal axe cleave with a 0.14-second windup, a cutting sweep through 0.32 seconds, and recovery through 0.72 seconds. `axe_swing.gd` shares the blade path and timing with damage detection; each target can be hit only once per stroke, and walls/closed doors still prevent hits. `axe_motion.gd` aligns the animated hand with the wrapped grip. Movement can continue while attack facing stays fixed.

Eleven [environment assets](../sdk/examples/gauntlet/assets/environment/README.md) replace prototype chests, flasks, food, keys and summoning plinths, and add braziers, urns, barrels, reliefs, grates and beveled masonry. Existing scanned materials and static batches remain. Room floors vary between mosaics, fringed runners, drains and worn stone; decoration stays off locked passages and introduces no movement blockers.

`party_visibility.gd` combines current sight from up to sixteen non-escaped players. The seven-tile clear radius and three-tile fade remain, but **walls and closed doors stop sight**, including exact diagonal corners. The blocking surface itself remains readable. Fallen teammates keep their sight; escaped/disconnected players lose theirs. All-party completion leaves the exit visible. There is no explored-cell history: moving away hides the room again.

Eight angular wedges project solid cells into shadow intervals. Grid rays handle overlapping octant boundaries. A bounded cache of 64 static fields avoids rebuilding unchanged geometry; opening doors or changing maps invalidates it. Native image blending combines current fields into a small mask, while the shader follows exact player positions for the soft radial edge. The cache contains computed geometry, not persistent exploration, and only the current party contributes to the displayed mask.

The overlay runs inside the dungeon viewport and reconstructs each visible surface's world X/Z from its depth, so raised heads, walls and props use their real location. Fog noise uses a separate floor projection so it cannot outline hidden geometry. Hidden enemies, pickups, gates, generators, portal and effects are also culled with their labels and health bars. The HUD, lobby and pause menu remain clear. Both minimap and full-dungeon overview respect current sight. Simulation/AI continue outside sight. [Godot depth-based post-processing](https://docs.godotengine.org/en/stable/tutorials/shaders/advanced_postprocessing.html).

Automated checks compare the shadow field with independent grid-crossing rays on all three real maps, test closed/open doors and diagonal corners, and verify hidden models disappear and return after the real unlock path. Cast checks compare the actual axe blade against simulation coordinates through four moving/facing directions and verify the hand stays on its grip. Rendered reviews include windup/contact/recovery poses and gathered/separated/full-party fog views. Sustained frame-rate and physical gameplay acceptance remain playtest work.

Validation note: the full headless SDK suite and Metal render runs completed cleanly. A supplementary Compatibility-renderer run completed all 71 visibility assertions but Godot reported two 349,524-byte GL texture leaks at shutdown. Compatibility teardown therefore remains unqualified; this is separate from the clean default Metal path used for dogfooding.
