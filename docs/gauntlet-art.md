...
`party_visibility.gd` combines current sight from up to sixteen non-escaped players. Each player has a seven-tile clear radius and three-tile fade. **Walls and doors do not block sight**: nearby rooms remain visible inside these circular areas. Fallen teammates keep their sight; escaped/disconnected players lose theirs. All-party completion leaves the exit visible. There is no explored-cell history: moving away hides the room again.

Visibility uses distance to the nearest current player, with the same smooth radial falloff in the shader and minimap. No wall rays, door masks or visibility caches are needed.
...
Automated checks cover radius/fade, sixteen separated players, movement, disconnects and completion. Live-scene checks verify nearby enemies and keys stay visible across walls and closed/open doors, then disappear when the party moves out of range. Cast checks compare the actual axe blade against simulation coordinates through four moving/facing directions and verify the hand stays on its grip. Rendered reviews include windup/contact/recovery poses and gathered/separated/full-party fog views. Sustained frame-rate and physical gameplay acceptance remain playtest work.

Validation note: the radial-only revision passed focused Gauntlet checks (58 visibility, 65 cast, 237 gameplay, 103 audio, 374 campaign and process return) and clean Metal renders. The full SDK run stopped on the unrelated Sunbreak movement assertion; it is not recorded as a full-suite pass. Before this revision, a supplementary Compatibility-renderer run completed its 71 visibility assertions but Godot reported two 349,524-byte GL texture leaks at shutdown. Compatibility teardown remains unqualified.
